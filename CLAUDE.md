...
- `web/src/app/api/feedback/chat/route.ts` — same `claudeSessions` infra but with a 10-minute turn timeout (Pimberton may run `npm ci`, install playwright, spin up a prototype). System prompt comes from `feedback/prompt.md` with `{{repo}}` / `{{repoRoot}}` / `{{sessionDir}}` substituted. Per-session sandbox lives at `$STUDY_DIR/feedback/drafts/<sessionId>/`. `DELETE` cleans up the session AND any prototype sandbox under `/Users/julianmoncarz/tutorin-wt/` (kills `next dev` on the recorded port via `pkill`, then `rm -rf`s the sandbox path — guarded by a hardcoded prefix check).
